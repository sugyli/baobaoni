...
@section('title'){{ $chapter['chaptername'] }}_{{$chapter['articlename']}}-{{config('app.name')}}-{{config('app.url')}}@endsection
...
.reader_top_nav .reader__back {
  ...
  width: 23px;
  height: 23px;
  background: url(/images/@EaSXPML.png) no-repeat;
...
<div v-bind:style="'min-height:'+screen_height +'px;'">
    ...
    <div id="top_nav" class="reader_top_nav" style="display: none;">
        ...
        <a class="reader__more" href="/"><i class="iconfont icon-shouye1"></i></a>
    </div>
    </user-report>
    ...
    <div class="m-button-bar">
        <ul class="u-tab">
            <a href="{{ $previousChapter['link'] or $chapter['link'] }}" target="_top" title="{{ $previousChapter['chaptername'] or  $chapter['articlename']}}">
              <li>上一章</li>
            </a>
            <a href="{{ $nextChapter['link'] or $chapter['link'] }}" target="_top" title="{{ $nextChapter['chaptername'] or  $chapter['articlename'] }}">
              <li>下一章</li>
            </a>
        </ul>
    </div>
    <div class="m-gongneng-bk" id="gongneng-container" style="display: none;">
      ...
      <div class="child-mod">
          <span>功能2</span>
          <button class="font-size-button" onclick="window.location.href='{{route('novel.outboxs.create')}}?title=书名：{{ $chapter['articlename']}}_章节名：{{ $chapter['chaptername']}}&amp;from=来路：{{request()->url()}}&amp;redirect_url={{request()->url()}}'">举报</button>

          <button class="font-size-button" onclick="window.location.href='{{route('novel.bookshelf.index')}}?redirect_url={{request()->url()}}'">书架</button>
      </div>
    </div>
    ...
    <div class="bottom-nav-bk bottom_nav" style="display: none;">
      <div class="reader__ft-bar">
        <a href=" {{ $previousChapter['link'] or $chapter['link'] }}" target="_top" title="{{ $previousChapter['chaptername'] or  $chapter['articlename'] }}">上一章</a>
        <a href="javascript:"></a>
        <a href="{{ $nextChapter['link'] or $chapter['link'] }}" target="_top" title="{{ $nextChapter['chaptername'] or  $chapter['articlename'] }}">下一章</a>
      </div>
      <a class="reader__ft" href="{{ route('novel.mulu',['bid'=>$chapter['articleid'] ]) }}" target="_top">
