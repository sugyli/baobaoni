...
@section('title'){{ $bookData->articlename }}全文阅读_{{ $bookData->articlename }}最新章节-{{get_sys_set('wabname')}}-{{get_sys_set('waburi')}}@endsection
...
@section('description'){{ $bookData->articlename }}是由{{ $bookData->author }}所写的{{$bookData->getSort()['title'] or '未知分类'}}类小说，本站提供{{ $bookData->articlename }}最新章节观看,{{ $bookData->articlename }}全文阅读等服务，如果您发现{{ $bookData->articlename }}更新慢了,请第一时间联系{{get_sys_set('wabname')}}。@endsection

@section('content')
<mheader
  left_icon = 'icon-fanhui1'
  left_href = '/'
  right_icon = 'icon-shoucang'
  right_href ='/'
  title = '书城书城书城书城书城书城书城书城书城书城书城'
