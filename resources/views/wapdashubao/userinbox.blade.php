...
<div class="header online HeaderTitlePosition">
    收邮箱已存({{$user->relationInboxs->count()}})封
  ...
  <ul class="shoujian Displayanimation">
    @if($user->relationInboxs->count() > 0)
      ...
      <li>
        ...
        <p>标题：<a href="{{ route('member.inboxs.show',['id'=> $message->messageid ]) }}?redirect_url={{request()->url()}}">{{$message->title}}</a></p>
        ...
        <p><a class="red-bg" href="{{ route('member.inboxs.destroy',['checkid'=>$message->messageid]) }}">删除消息</a></p>
      </li>
      @endforeach
