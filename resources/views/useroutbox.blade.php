...
<ul class="fajian Displayanimation">
  @if($user->relationOutboxs->count() > 0)
    ...
    <li>
      ...
      <p>标题：<a href="{{ route('novel.outboxs.show',['id'=> $message->messageid ]) }}?redirect_url={{request()->url()}}">{{$message->title}}</a></p>
      <p>日期：{{ formatTime($message->postdate) }}</p>
      <p><a class="red-bg" href="{{ route('novel.outboxs.destroy',['id'=>$message->messageid]) }}">删除消息</a></p>
    </li>
    @endforeach
  @endif
</ul>
