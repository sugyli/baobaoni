...
<section class="list">
  ...
  <ul>
    @if ($searchDatas->count() > 0)
      ...
      <li>
        <a href="{{$searchData->link()}}" title="{{$searchData->articlename}}">
          <img src="{{ $searchData->imgflag }}" alt="{{$searchData->articlename}}">
        </a>
        <div class="title">
          <h3>
            <a href="{{$searchData->link()}}" title="{{$searchData->articlename}}">《{{$searchData->articlename}}》</a>
          </h3>
          <span class="author">作者：{{$searchData->author}}</span>
          <user-fenleibnt
              bid={{$searchData->articleid}}
              cid=0
              addbookcaseurl="{{route('webajax.bookshelf.addbookcase')}}"
          ></user-fenleibnt>

          <span class="read">
            <a href="{{$searchData->link()}}" title="{{$searchData->articlename}}">在线阅读</a>
          </span>
        </div>
        <div class="status">连载中<span>最新章节：<a href="{{route('web.articles.content', ['bid' => $searchData->articleid , 'cid' => $searchData->lastchapterid])}}">{{$searchData->lastchapter}}</a></span></div>
        <p>{{ $searchData->intro }}</p>
        <div class="data">
          <span>今日推荐：{{ $searchData->getDayHits() }}</span>
          <span>周推荐：{{ $searchData->getWeekHits() }}</span>
          <span>月推荐：{{ $searchData->getMonthHits() }}</span>
          <span>更新：{{formatTime($searchData->lastupdate)}}</span>
        </div>
      </li>
      @endforeach
    @endif
